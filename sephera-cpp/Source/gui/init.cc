...
void Init::setupGui(SepheraWindow *sepheraWindow) {
  ...
  components->setLineEdit(displayPathTextbox, sepheraWindow->layout, 150, 30, 4,
                         1);

  components->setLabel(displayPathDescription, sepheraWindow->layout, 150, 30, 3,
                      1, QString("Input project path or section project"));

  components->setPushButton(sepheraWindow, open_project_btn,
                           sepheraWindow->layout, 30, 30, 4, 2, "Open Project");

  components->setPushButton(sepheraWindow, scan_project_btn,
                           sepheraWindow->layout, 30, 30, 4, 3, "Scan Project");

  components->setPushButton(sepheraWindow, option_btn, sepheraWindow->layout, 30,
                           30, 0, 3, "Options");
}

void Init::setupContext() { context->setOptionMenuBehavior(this->option_btn); }
