...
from benchmark_render import render_markdown_report, render_terminal_summary

DATASET_CHOICES: Final[tuple[DatasetName, ...]] = (
	"repo",
	"small",
	"medium",
	"large",
...
def parse_args() -> BenchmarkArgs:
	parser = ArgumentParser(description = "Benchmark the Sephera Rust CLI")
	parser.add_argument(
		"--datasets",
		nargs = "+",
...
def benchmark_case(
	...
	rust_command = loc_command(executable = rust_cli, case = case)
	rust_stats = measure_command(command = rust_command, warmup = warmup, runs = runs)
...
def measure_command(command: list[str], warmup: int, runs: int) -> CommandStats:
	...
	return CommandStats(
		...
		summary = parse_output_summary(last_completed.stdout),
	)
...
def parse_output_summary(stdout: str) -> OutputSummary | None:
